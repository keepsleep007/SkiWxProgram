				...
				<scroll-view class="list-scroll-content" scroll-y @scrolltolower="loadData">
					...
					<view v-for="(item,index) in tabItem.orderList" :key="index" class="order-item">
						...
						<view v-if="item.goodsList.length === 1" class="goods-box-single" v-for="(goodsItem, goodsIndex) in item.goodsList" :key="goodsIndex" @click="toOrderInfo(item)">
							...
							<view class="right">
								<text class="title clamp">预约时间：{{goodsItem.startDate}} ~ {{goodsItem.endDate}}</text>
								<template v-for="(teachMode, index) in teachModeList">
									<template v-if="goodsItem.tcMode==teachMode.dictValue">
										<text :key="index" class="attr-box">教学模式：{{teachMode.dictLabel}}，教学时长：{{goodsItem.tcTime}}小时</text>
									</template>
								</template>
							</view>
						</view>
...
<script>
	...
	import empty from "../active/empty";
	export default {
		components: {
			empty
		},
		...
		methods: {
			...
			async loadDataForBackground(tabCurrentIndex, pageNum, pageSize) {
				...
				await this.request('/app_api/teacher/getTeacherOrderList', requestParam, 'GET').then(res => {
					if (res) {
			...
			dialogConfirm() {
				...
				this.request('/app_api/teacher/cancelTeacherOrder', requestParam, 'POST').then(res => {
					if (res) {
